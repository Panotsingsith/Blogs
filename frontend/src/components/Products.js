...
export default class Products extends Component {
    ...
    submit(event, id) {
        ...
        if (id === 0) {
            axios.post(`http://localhost:8000/api/product/`, { name: this.state.name, email: this.state.email, password: this.state.password })
                .then(() => {
                    this.componentDidMount();
                })

        } else {
            axios.put(`http://localhost:8000/api/product/${id}`, { name: this.state.name, email: this.state.email, password: this.state.password })
                .then(() => {
                    this.componentDidMount();
                })
        }
