...
export default class Products extends Component {
    ...
    submit(event, id) {
        ...
        if (id === 0) {
            axios.post(`http://localhost:8000/api/category/`, { name: this.state.name, email: this.state.email, password: this.state.password })
                .then(() => {
                    this.componentDidMount();
                })

        } else {
            axios.put(`http://localhost:8000/api/category/${id}`, { name: this.state.name, email: this.state.email, password: this.state.password })
                .then(() => {
                    this.componentDidMount();
                })
        }
